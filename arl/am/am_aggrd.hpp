...
alignas(alignof_cacheline) gex_AM_Index_t hidx_generic_amaggrd_reqhandler;
alignas(alignof_cacheline) gex_AM_Index_t hidx_generic_amaggrd_ackhandler;
void generic_amaggrd_reqhandler(gex_Token_t token, void *buf, size_t nbytes,
...
void generic_amaggrd_reqhandler(gex_Token_t token, void *void_buf, size_t unbytes,
                                gex_AM_Arg_t t0, gex_AM_Arg_t t1, gex_AM_Arg_t t2, gex_AM_Arg_t t3) {
  using req_invoker_t = int(intptr_t, char*, int, char*, int);

  char* in_buf = static_cast<char*>(void_buf);
  int in_nbytes = static_cast<int>(unbytes);
  ARL_Assert(in_nbytes >= (int) sizeof(AmaggrdReqMeta), "(", in_nbytes, " >= ", sizeof(AmaggrdReqMeta), ")");
  AmaggrdReqMeta in_meta;
  gex_AM_Arg_t* t = reinterpret_cast<gex_AM_Arg_t*>(&in_meta);
...
//  printf("recv meta: %ld, %ld\n", meta.fn_p, meta.type_wrapper_p);
  ...
  gex_AM_ReplyMedium2(token, hidx_generic_amaggrd_ackhandler, out_buf, out_consumed, GEX_EVENT_NOW, 0,
                      out_t[0], out_t[1]);
...
void generic_amaggrd_ackhandler(gex_Token_t token, void *void_buf, size_t unbytes,
                                gex_AM_Arg_t t0, gex_AM_Arg_t t1) {
...
void init_amaggrd() {
  hidx_generic_amaggrd_reqhandler = am_internal::gex_am_handler_num++;
  hidx_generic_amaggrd_ackhandler = am_internal::gex_am_handler_num++;
  ARL_Assert(am_internal::gex_am_handler_num < 256, "GASNet handler index overflow!");

  gex_AM_Entry_t htable[2] = {
      { hidx_generic_amaggrd_reqhandler,
          (gex_AM_Fn_t) generic_amaggrd_reqhandler,
          GEX_FLAG_AM_MEDIUM | GEX_FLAG_AM_REQUEST,
          4 },
      { hidx_generic_amaggrd_ackhandler,
          (gex_AM_Fn_t) generic_amaggrd_ackhandler,
...
//          printf("rank %ld send %p, %d\n", rank_me(), std::get<0>(result), std::get<1>(result));
          ...
          gex_AM_RequestMedium4(backend::tm, i, amaggrd_internal::hidx_generic_amaggrd_reqhandler,
...
//  printf("sizeof(payload): %lu\n", sizeof(Payload));
  ...
  if (std::get<0>(result) != nullptr) {
    if (std::get<1>(result) != 0) {
      gex_AM_Arg_t* t = reinterpret_cast<gex_AM_Arg_t*>(amaggrd_internal::global_meta_p);
      gex_AM_RequestMedium4(backend::tm, remote_proc, amaggrd_internal::hidx_generic_amaggrd_reqhandler,
                            std::get<0>(result), std::get<1>(result), GEX_EVENT_NOW, 0,
                            t[0], t[1], t[2], t[3]);
    }
    delete [] std::get<0>(result);
