...
void worker() {
  ...
  register_amaggrd(empty_handler, req_payload);

  for (int i = 0; i < num_ops; ++i) {
    int target_rank = distribution(generator);
    auto future = rpc_aggrd(target_rank, empty_handler, req_payload);
  ...
  print("Total single-direction node bandwidth (req/gross): %.2lf MB/s\n",
      sizeof(AmaggrdReqPayload<ReqPayload>) * num_ops * local::rank_n() * 2 / duration_total);
  ...
  print("Total single-direction node bandwidth (ack/gross): %.2lf MB/s\n",
      sizeof(AmaggrdAckPayload<AckPayload>) * num_ops * local::rank_n() * 2 / duration_total);
