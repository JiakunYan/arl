...
def draw_gex():
    ...
    for a, b1, b2 in zip(labels, values1, values2):
        ax.text(a, b1, '%d' % b1, ha='center', va='bottom', fontsize=11)
        ax.text(a, b1 + max(b2, 100), '%d' % (b1+b2), ha='center', va='bottom', fontsize=11)
    ax.legend((p1[0], p2[0]), ("GASNet overhead (us)", "time inside handler (us)"))
    # plt.show()
    plt.savefig("draw/heavy_handler_gex.png")
    return labels, values1, values2
